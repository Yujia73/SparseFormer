...
class SparseFormer(nn.Module):
    def __init__(self, num_classes=6, drop_rate=0.4, normal_init=True, pretrained=False,attn1='CA',attn2='CA'):
        ...
        self.aspp = ASPP(output_stride=16, norm_fn=norm_fn_for_extra_modules,inplanes=232)
        self.decoder1 = Decoder_Attention(num_classes, 48, norm_fn_for_extra_modules, attention_mode=self.attn1)
        self.decoder2 = Decoder_Attention(num_classes, 48, norm_fn_for_extra_modules, attention_mode=self.attn2)
        ...
        self.final_1 = nn.Sequential(
            ...
            Conv(64, self.num_classes, 3, bn=False, relu=False)
        )
        ...
        self.final_3 = nn.Sequential(
            ...
            Conv(32,self.num_classes, 3, bn=False, relu=False)
        )
    ...
    def forward(self, imgs):
        ...
        x_u64 = self.efficienet.blocks[1](x_u128)#4,32,128,128

        x_u_2 = self.efficienet.blocks[2](x_u64)
        x_low_level = x_u_2
        x_u_2 = self.drop(x_u_2) #4,48,64,64

        x_u_3 = self.efficienet.blocks[3](x_u_2)
        x_u_3 = self.drop(x_u_3)

        x_u_3 = self.efficienet.blocks[4](x_u_3)
        x_u_3 = self.drop(x_u_3)#4,136,32,32

        x_u = self.efficienet.blocks[5](x_u_3)
        #x_u = self.drop(x_u)#4,232,16,16


        out = self.aspp(x_u)
        x_main = self.decoder1(out, x_low_level)
        x_1 = resize_for_tensors(x_main, imgs.size()[2:], align_corners=True) #cnn output1

        x_aux= self.decoder2(out, x_low_level)
        x_2 = resize_for_tensors(x_aux, imgs.size()[2:], align_corners=True) #cnn output2       
