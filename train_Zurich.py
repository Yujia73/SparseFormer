...
from torch.utils import data
from dataloaders.ZurichDataset import ZurichDataSet,ZurichTestDataSet
...
name_classes = np.array(['Road','Build','Tree','Grass','Bardland','Water','Railway','Pool','Bg'], dtype=np.str_) #Zurich数据集
epsilon = 1e-14
...
parser.add_argument("--data_name", type=str, default='point_an1_Zurich_p0.5_r0.3_all',
                        help="dataset path.")
parser.add_argument("--model_name", type=str, default='SparseFormer',
                        help="dataset path.")

parser.add_argument("--data_dir", type=str, default='./data/Zurich/',
                        help="dataset path.")
parser.add_argument("--test_dir", type=str, default='./data/Zurich/',
                        help="dataset path.")
parser.add_argument("--train_list", type=str, default='./data/Zurich/train.txt',
                        help="training list file.")
parser.add_argument("--test_list", type=str, default='./data/Zurich/test.txt',
                        help="test list file.")  
...
parser.add_argument("--snapshot_dir", type=str, default='./Result_Zurich/',
...
def main():
    ...
    trainloader = data.DataLoader(
                       ZurichDataSet(args.data_dir, args.train_list,
                        crop_size=(512,512),set=args.sup_type,mode=0,id= args.sup_id),
                        batch_size=args.batch_size, shuffle=True, num_workers=0, pin_memory=True)

    test_loader = data.DataLoader(
                        ZurichTestDataSet(args.test_dir, args.test_list, set='test'),
    ...
    for epoch in range(50):
        ...
        for batch_index,src_data in enumerate(tqdm(trainloader)):
            ...
            lbl_gt = gt_batch.detach().cpu().numpy() #ground truth
